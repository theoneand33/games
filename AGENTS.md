...
```
├── src/
│   ├── components/       Reusable Astro components (gametile, flash)
│   ├── data/             SEO metadata for games (game-seo.ts)
│   ├── layout/           Single layout (layout.astro — handles game pages, home, DMCA)
...
│   ├── games/run3/       HTML5 game bundle for Run 3
│   ├── images/           Cover art and logo assets
│   └── ruffle/           Ruffle WebAssembly Flash emulator
├── astro.config.mjs      Astro config (prefetch, sitemap, Tailwind)
...
- **React** — used via `@astrojs/react` integration (now removed — no React components in use)
